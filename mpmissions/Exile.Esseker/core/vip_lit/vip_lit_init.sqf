...
if (count _mapSubs > 0) then {_scriptMapSubs = [_respawn, _mapSubs] execVM "core\vip_lit\vip_lit_mapSubtractions.sqf"; waitUntil {scriptDone _scriptMapSubs}};
if (count _mapAdds > 0) then {_scriptMapAdds = [_respawn, _mapAdds] execVM "core\vip_lit\vip_lit_mapAdditions.sqf"; waitUntil {scriptDone _scriptMapAdds}};
if (count _nvg > 0) then {[_respawn, _nvg] call compile preProcessFileLineNumbers "core\vip_lit\vip_lit_NVG.sqf"};
